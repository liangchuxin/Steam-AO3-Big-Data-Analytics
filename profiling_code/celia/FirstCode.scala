...
val steamDf = spark.read.parquet(steamPath)
...
// =====================================
println("===== STEAM NUMERIC COLUMNS =====")
numericCols.foreach(println)

println("===== STEAM MEAN OF NUMERICAL COLUMNS =====")
val meanExprs = numericCols.map(c => avg(col(c)).alias(c + "_mean"))
steamDf.select(meanExprs: _*).show(false)

println("===== STEAM MEDIAN OF NUMERICAL COLUMNS =====")
val medianExprs = numericCols.map(c =>
  expr(s"percentile_approx($c, 0.5)").alias(c + "_median")
)
steamDf.select(medianExprs: _*).show(false)

println("===== STEAM MODE OF NUMERICAL COLUMNS =====")
numericCols.foreach { c =>
  println(s"--- Mode for column: $c ---")
  steamDf.groupBy(col(c)).count().orderBy(desc("count"), asc(c)).show(5, false)
}

println("===== STEAM STANDARD DEVIATION OF RATING =====")
steamDf.select(stddev(col("rating")).alias("rating_stddev")).show(false)


println("===== STEAM MIN OF NUMERICAL COLUMNS =====")
val minExprs = numericCols.map(c => min(col(c)).alias(c + "_min"))
steamDf.select(minExprs: _*).show(false)

println("===== STEAM MAX OF NUMERICAL COLUMNS =====")
val maxExprs = numericCols.map(c => max(col(c)).alias(c + "_max"))
steamDf.select(maxExprs: _*).show(false)


// Code Cleaning, Text formatting & Creating binary column for "rating"
// =====================================
val dfTextCleaned = steamDf.withColumn("name_clean", lower(col("name"))).withColumn("developer_clean", lower(col("developer"))).withColumn("publisher_clean", lower(col("publisher")))

println("===== STEAM TEXT CLEANING SAMPLE =====")
...
println("===== STEAM DISTRIBUTION OF is_top_rated =====")
